		...
		<div class="card-body">
			<table class="table tabe-hover table-bordered" id="list">
				...
				<tbody>
					<?php
					$i = 1;
					$type = array('', "Admin", "Alumno");
					$stmt = $conn->query("SELECT *, CONCAT(lastname,', ',firstname,' ',middlename) as name FROM users ORDER BY CONCAT(lastname,', ',firstname,' ',middlename) ASC");
...
<script>
	...
	$('.view_user').click(function(){
		uni_modal("<i class='fa fa-id-card'></i> Detalles del usuario","view_user.php?id="+$(this).attr('data-id'))
	})
	$('.delete_user').click(function(){
	_conf("¿Está seguro de eliminar este usuario?","delete_user",[$(this).attr('data-id')])
	})
	})
